...
class InvoiceService {
  static async list(
    ...
    if (status) {
      ...
      if (operator) {
        ...
        // Sin operator, usar = por defecto de forma segura
  ...
  private static readonly INVOICES_BASE_DIR = path.resolve(
    __dirname,
    "../../invoices"
  );
  ...
  private static isValidFileName(fileName: string): boolean {
    ...
    return (
      !invalidPatterns.some((pattern) => pattern.test(fileName)) &&
      /^[a-zA-Z0-9_\-\.]+\.pdf$/.test(fileName)
    ); // Solo nombres seguros con extensión .pdf
  }
  ...
  private static isAllowedPaymentDomain(domain: string): boolean {
    try {
      ...
      return (
        this.ALLOWED_PAYMENT_DOMAINS.has(hostname) ||
